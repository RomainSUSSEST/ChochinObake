...
public class SpectralFluxAnalyzer
{
    ...
    public List<SpectralFluxInfo> trueBeats;

    public bool fistBeats = true;
    ...
    public SpectralFluxAnalyzer()
    {
        ...
        trueBeats = new List<SpectralFluxInfo>();
    ...
    public void analyzeSpectrum(float[] spectrum, float time)
    {
        ...
        if (spectralFluxSamples.Count >= thresholdWindowSize)
        {
            ...
            if (curPeak && fistBeats)
            {
                spectralFluxSamples[indexToDetectPeak].isPeak = true;
                trueBeats.Add(spectralFluxSamples[indexToDetectPeak]);
                fistBeats = false;
            }
            if(!curPeak)
                fistBeats = true;
    ...
    {
       // Debug.Log(spectralFluxSamples[spectralFluxIndex].prunedSpectralFlux + " > " + spectralFluxSamples[spectralFluxIndex + 1].prunedSpectralFlux+
